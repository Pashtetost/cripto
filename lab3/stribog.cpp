...
void stribog_X(container first, container second, container result){
...
//S-преобразование
void stribog_S(container input){
   uint8_t Pi[]={// S-блок
...
//P-преобразование
void stribog_P(container input){
   int Tau[]={ // значения перестановки
   ...
   container internal; // необходимо сделать промежуточный вектор, так как используем каждую итерацию значения входного
   // будем переписывать сразу значения входного, то будем в последних итерациях брать значения, которые уже сменили
   // из-за чего получим не верный результат
   for (int i = 0; i < BLOCK_SIZE; i++){
       internal[i] = input[Tau[i]]; // выполнение перестановки элементов
   }
   memcpy(input, internal, BLOCK_SIZE);
}

//L-преобразование
void stribog_L(container input){
   unsigned long long A[64] = { // матрица А
   ...
   unsigned long long value = 0; // 64-х битное значение

   for(int i=0;i<8;i++) // 8 раз необходимо выполнить итерацию
   {
       value=0;
       for(int k=0;k<8;k++) // по факту делим исходный вектор на восемь восмибайтовых
       {
           for(int j=0;j<8;j++)
           {
               if ((input[i*8+k] & (1<<(7-j))) != 0) // Если очередной бит равен 1, то
                   value ^= A[k*8+j]; // ксорим предыдущее значение с значением из матрицы
           }
       }
       for(int k=0;k<8;k++)
       {
           input[i*8+k] = (value & ((unsigned long long)0xFF << (7-k)*8)) >> (7-k)*8; // заполняем байты входного вектора
       }
   }
}

//Исключающее или
void stribog_Add(container first, container second, container result){
 uint16_t internal = 0;
 for (int i = BLOCK_SIZE - 1; i >= 0; i--)
 {
   internal = first[i] + second[i] + (internal >> 8);
   result[i] = internal & 0xff;
 }
}

//Генерация раундового ключа
void stribog_GetKey(container K, int i)
{
   container C[12] = { //итерационные константы
       {
...
//Функция E
void stribog_E(container K, container input, container output){
 memcpy(K, K, BLOCK_SIZE);
 stribog_X(input, K, output);
 for(int i = 0; i < 12; i++)
 {
   stribog_S(output);
   stribog_P(output);
   stribog_L(output);
   stribog_GetKey(K, i);
   stribog_X(output, K, output);
 }
}

//Функция сжатия G
void stribog_G(container h, container N, container m)
{
   container K, internal;
   stribog_X(N, h, K);
   ...
   stribog_E(K, m, internal);

   stribog_X(internal, h, internal);
   stribog_X(internal, m, h);
}

//Дополнение блока к виду 00 .. 00 01 ..
void stribog_Padding(container input, unsigned int size){
   container internal; // Промежуточный вектор
   for (int i = 0 ; i < BLOCK_SIZE ; i++){ // Обнуляем промежуточный вектор
       internal[i] = 0x00;
   }
   for (unsigned int i = 0 ; i < size ; i++){
       internal[(BLOCK_SIZE - size)+i] = input[i];
   }
   // в промежуточный вектор
   internal[BLOCK_SIZE - size-1] = 0x01; // Добавляем в нужное место единичку
   for (int i = 0 ; i < BLOCK_SIZE ; i++){ // Кладем все, что получилось, обратно
       input[i] = internal[i];
   }
}

//Этап 1 //Инициализация базовой структуры
void stribog_Init512(Context *Context){
   for (int i = 0 ; i < BLOCK_SIZE ; i++){// Обнуляем все переменные базовой структуры
       Context->buffer[i]=0x00;
       Context->hash[i]=0x00;
       Context->h[i]=0x00;
       Context->N[i]=0x00;
       Context->Sigma[i]=0x00;
       Context->v_0[i]=0x00;
       Context->v_512[i]=0x00;
   }
   Context->hash_size = BLOCK_SIZE;
   Context->v_512[62] = 0x02; // Инициализируем вектор v_512
}
//Этап 1 //Инициализация базовой структуры
void stribog_Init256(Context *Context){
   for (int i = 0 ; i < BLOCK_SIZE ; i++){// Обнуляем все переменные базовой структуры
       Context->buffer[i]=0x00;
       Context->hash[i]=0x00;
       Context->h[i]=0x01;
...
//Этап 2
void stribog_2(Context *Context){
 stribog_G(Context->h, Context->N, Context->buffer);
 stribog_Add(Context->N, Context->v_512, Context->N);
 stribog_Add(Context->Sigma, Context->buffer, Context->Sigma);
}

//Этап 3
void stribog_3(Context *Context){
   container internal;
   for (int i = 0 ; i < BLOCK_SIZE ; i++){
       internal[i]=0x00;
   }
   // Формируем строку с размером сообщения
   internal[62] = ((Context->buf_size * 8) >> 8) & 0xff;
   internal[63] = (Context->buf_size * 8) & 0xff;

   stribog_Padding(Context->buffer, Context->buf_size); // Дополняем оставшуюся часть до полных 64 байт

   stribog_G(Context->h, Context->N, Context->buffer);

   // Формируем контрольную сумму сообщения
   stribog_Add(Context->N, internal, Context->N);
   stribog_Add(Context->Sigma, Context->buffer, Context->Sigma);

   stribog_G(Context->h, Context->v_0, Context->N);
   stribog_G(Context->h, Context->v_0, Context->Sigma);
   for (int i = 0 ; i < BLOCK_SIZE ; i++){// Записываем результат в нужное место
       Context->hash[i]=Context->h[i];
   }
}
...
uint8_t* stribog_512(uint8_t data[]){//входное сообщение/файл
   Context Context;
   stribog_Init512(&Context); // проводим инициализацию
   unsigned int data_size = _msize(data)/sizeof(data[0]); // размер входных данных
   Context.buf_size=BLOCK_SIZE; // принимаем размер сообщения для шага как размер блока
   // при шаге 2 он не меняется, а при попадании на шаг 3 примет значение ниполного блока
   while (1){ // пока не попадем на шаг 3 и там не сработает break
       if(data_size > BLOCK_SIZE-1){ // если размер данных больше одного блока выполняем шаг 2
           for(unsigned int i = 0 ; i < BLOCK_SIZE ; i++){
               Context.buffer[i]=data[data_size  - BLOCK_SIZE + i];
           }
           data_size = data_size - BLOCK_SIZE; // меняем размер входных данных, которые мы еще не обработали
           stribog_2(&Context); // выполняем шаг 2
       }else{ // иначе выполняем шаг 3
           for(int i = 0 ; i < BLOCK_SIZE ; i++){ // обнуляем буфер с вектором в котором будет блок остатка входных данных
               Context.buffer[i]=0x00;
           }
           for(unsigned int i = 0 ; i < data_size ; i++){ // записываем остаток
               Context.buffer[i]=data[i];
           }
           Context.buf_size=data_size;  // говорим что сейчас размер блока неполный
           stribog_3(&Context); // выполняем шаг 3
           data_size = 0; // говорим что все обработали
           break; // выходим из while
       }
   }
   uint8_t *result = new uint8_t[BLOCK_SIZE]; // формируем результат и возвращаем его
   for(int i = 0 ; i < BLOCK_SIZE ; i++){
       result[i]=Context.hash[i];
   }
   return result;
}
...
uint8_t* stribog_256(uint8_t data[]){//входное сообщение/файл
   // все по анологии как и в случае 512 бит
   Context Context;
   stribog_Init256(&Context); // но инициализация иная
   unsigned int data_size = _msize(data)/sizeof(data[0]);
   while (1){
       if(data_size > BLOCK_SIZE-1){
           for(unsigned int i = 0 ; i < BLOCK_SIZE ; i++){
               Context.buffer[i]=data[data_size  - BLOCK_SIZE + i];
           }
           data_size = data_size - BLOCK_SIZE;
           Context.buf_size=BLOCK_SIZE;
           stribog_2(&Context);
       }else{
           for(int i = 0 ; i < BLOCK_SIZE ; i++){
               Context.buffer[i]=0x00;
           ...
           Context.buf_size=data_size;
           stribog_3(&Context);
           data_size = 0;
           break;
       }
   }
   uint8_t *result = new uint8_t[BLOCK_SIZE/2]; // и результат в два раза меньше
